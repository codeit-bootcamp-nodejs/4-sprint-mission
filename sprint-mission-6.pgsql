...
select pizza_id, sum(quantity) as "총 주문 수" 
from order_details 
group by pizza_id;
...
select o.date, count(distinct od.order_id) as "order_count", sum(od.quantity) as "total_quantity"
...
--     3. `order`의 `id`가 78에 해당하는 주문 내역들을 조회합니다. 주문 내역에서 각각 주문한 피자의 이름을 `pizza_name`, 피자의 크기를 `pizza_size`, 피자 가격을 `pizza_price`, 수량을 `quantity`, 각 주문 내역의 총 금액을 `total_amount` 라는 이름으로 조회해 주세요.
        select pt.name as "pizza_name", p.size as "pizza_size", p.price as "pizza_price", od.quantity as "quantity", (od.quantity * p.price) as "total_amount"
        from orders as o
        join order_details as od
        on o.id = od.order_id
        join pizzas as p 
...
-- 
    ...
    -- 5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        select pt.name, sum(od.quantity * p.price) as "총 수익"
        from order_details as od
        join pizzas as p
        on od.pizza_id = p.id
        join pizza_types as pt
