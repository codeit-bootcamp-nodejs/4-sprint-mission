...
export class ProductsController {
  ...
  createProduct = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const createProductDto: CreateProductDto = req.body; // DTO 사용
      const user = req.user;
      if (!user) {
      ...
      if (!createProductDto.name || !createProductDto.content) { // DTO 필드 유효성 검사
        return res.status(400).json({ message: '상품명과 내용을 모두 입력해주세요.' });
      }

      const newProduct = await this.productsService.createProduct(createProductDto, userId); // DTO 전달
  ...
  updateProduct = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { productId } = req.params;
      const updateProductDto: UpdateProductDto = req.body; // DTO 사용
      const user = req.user;
      if (!user) {
        return res.status(401).json({ message: '인증 정보가 없습니다.' });
      }
      ...
      const updatedProduct = await this.productsService.updateProduct(+productId, userId, updateProductDto); // DTO 전달
