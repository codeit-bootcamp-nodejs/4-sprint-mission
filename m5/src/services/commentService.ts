...
export const createComment = async ({ resourceType, resourceId, content, userId }: CreateCommentServiceData) => {
    return prisma.$transaction(async (tx) => {
        // 1. resourceType에 따라 Post 또는 Product 존재 여부 확인
        if (resourceType === 'post') {
            const postExists = await tx.post.findUnique({ where: { id: parseInt(resourceId.toString(), 10) } });
            if (!postExists) {
                throw new CustomError('해당 게시글을 찾을 수 없습니다.', 404);
            }
        } else if (resourceType === 'product') {
            const productExists = await tx.product.findUnique({ where: { id: parseInt(resourceId.toString(), 10) } });
            if (!productExists) {
