...
async function getArticles(req: Request, res: Response, next: NextFunction) {
  ...
  const where: Prisma.ArticleWhereInput = keyword //title과 content 에서 원하는 keyword가 있는 데이터를 찾도록 만든 변수
    ? {
        OR: [
          {
            title: {
...
async function modifyArticle(req: Request, res: Response, next: NextFunction) {
  if (!req.user) return res.status(401).json({ message: "Unauthorized" });
...
async function getDetailArticle(req: Request, res: Response, next: NextFunction) {
  const articleId = Number(req.params.id);
