...
async function getProducts(req: Request, res: Response, next: NextFunction) {
  ...
  const where: Prisma.ProductWhereInput = keyword //name과 description에서 원하는 keyword가 들어간 데이터를 찾도록 만든 변수
    ? {
        OR: [
          {
            name: {
              contains: keyword,
              mode: "insensitive" as const, //대소문자 구분 없이 검색하기 위해
            },
          },
          {
            description: {
...
async function getDetailProduct(req: Request, res: Response, next: NextFunction) {
  const productId = Number(req.params.id);
