...
SELECT * FROM public.orders
WHERE date = '2025-03-12' AND time < '12:00:01';
...
WHERE pizza_id IN (SELECT id FROM public.pizzas WHERE price > 20);
...
FROM public.order_details AS o INNER JOIN public.pizzas AS p
ON o.pizza_id = p.id
GROUP BY o.pizza_id
...
/*
    1. 피자별(`pizzas.id` 기준) 판매 수량 순위에서 피자별 판매 수량 상위에 드는 베스트 피자를 10개를 조회해 주세요. `pizzas`의 모든 컬럼을 조회하면서 각 피자에 해당하는 판매량을 `total_quantity`라는 이름으로 함께 조회합니다.
        ...
        ```
*/
SELECT p.id, p.type_id, p.size, p.price, SUM(d.quantity) AS total_quantity
FROM pizzas AS p INNER JOIN order_details AS d
ON p.id = d.pizza_id
GROUP BY p.id
...
ON d.pizza_id = p.id
GROUP BY o.date
HAVING EXTRACT(MONTH FROM o.date) = 3 AND EXTRACT(YEAR FROM o.date) = 2025 ;
...
/*    
    4. `order_details`와 `pizzas` 테이블을 JOIN해서 피자 크기별(S, M, L) 총 수익을 계산하고, 크기별 수익을 출력하세요.
        ...
        ```
*/
SELECT p.size, SUM(d.quantity*p.price)
FROM order_details AS d INNER JOIN pizzas AS p
ON d.pizza_id = p.id
GROUP BY p.size;
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
SELECT t.name, SUM(p.price*d.quantity)
FROM order_details AS d INNER JOIN pizzas AS p
ON d.pizza_id = p.id
