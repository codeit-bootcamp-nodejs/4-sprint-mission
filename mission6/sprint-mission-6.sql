...
SELECT * FROM orders WHERE id = 432;
...
P.price
FROM pizzas AS P
LEFT JOIN order_details AS OD ON P.id = OD.pizza_id
WHERE P.price > 20;
...
ORDER BY COUNT_PIZZA ASC;

-- 6. `order_details` 테이블에서 피자별 총 수익을 `total_revenue` 라는 이름으로 구하세요. (수익 = `quantity * price`)
SELECT
TOTAL_PIZZA.pizza_id,
TOTAL_PIZZA.COUNT_PIZZA * P.price AS TOTAL_REVENUE
FROM
(SELECT
pizza_id,
SUM(quantity) AS COUNT_PIZZA
...
SUM(OD.quantity) AS TOTAL_QUANTITY
FROM pizzas AS P
RIGHT JOIN order_details AS OD ON P.id = OD.pizza_id
GROUP BY P.id
...
SELECT
DATE_ORDERS.date,
COUNT(DATE_ORDERS.order_id) AS TOTAL_ORDERS,
SUM(DATE_ORDERS.quantity * P.price) AS TOTAL_AMOUNT
FROM
(
SELECT
O.date,
...
PIZZA_ORDERS.quantity,
(PIZZA_ORDERS.quantity * P.price) AS TOTAL_AMOUNT
FROM pizzas AS P
INNER JOIN
(
SELECT
