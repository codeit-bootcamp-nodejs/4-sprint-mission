...
export class UserController {
    ...
    async modifyUserInfo(req:Request, res: Response, next: NextFunction){
        try {
            ...
            const result = await this.userService.modifyUserInfo({id:userId, nickname, email})
    ...
    async modifyUserPassword(req:Request, res: Response, next: NextFunction){
        try {
            ...
            const result = await this.userService.modifyUserPassword(userId, {currentPassword, newPassword})
            res.status(200).json({
                data: result
            })
