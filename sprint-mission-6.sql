...
SELECT o.date, SUM(od.order_id) AS order_count, SUM(od.quantity) AS total_quantity
FROM orders AS o
LEFT JOIN 
order_details AS od
ON o.id = od.order_id
...
SELECT date, COUNT(DISTINCT o.id) AS total_orders, SUM(od.quantity * p.price) AS total_amount
FROM orders AS o
...
SELECT pt.name AS pizza_name, p.size AS pizza_size, p.price AS pizza_price, SUM(od.quantity) AS quantity, SUM(od.quantity * p.price) AS total_amount
FROM order_details AS od
JOIN pizzas AS p
ON od.pizza_id = p.id 
...
GROUP BY pizza_name, p.size, p.price
ORDER BY pizza_size, pizza_price DESC;
...
SELECT p.size, SUM(od.quantity * p.price) AS total_amount
FROM pizzas AS p
JOIN order_details AS od
ON p.id = od.pizza_id
GROUP BY p.size
