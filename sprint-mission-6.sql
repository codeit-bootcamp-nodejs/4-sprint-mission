...
-- 3. `orders` 테이블에서 총 주문 건수를 `total_orders`라는 이름으로 구하세요.
    ...
    A. SELECT COUNT(id) AS total_orders 
       FROM orders;
...
-- 3. `pizzas` 테이블에서 `price`의 크기가 20보다 큰 피자의 종류만 `order_details` 테이블에서 조회하세요. (힌트: 서브쿼리)
    ...
    아래는 JOIN을 이용해서 작성해봤습니다 서브쿼리를 사용하는게 논리적으로 쉽고 보기 편하지만 JOIN을 이용하는것이 대용량에서는 성능 더 좋을 수 있음.
...
*/
    ...
    A. SELECT 
       ...
       WHERE o.id = 78
       GROUP BY pt.id, p.id
       ORDER BY p.size ASC, total_amount DESC;
