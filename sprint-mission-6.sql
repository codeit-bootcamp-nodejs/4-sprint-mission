...
/*
    2. `orders` 테이블에서 2025년 3월의 일별 주문 수량을 `total_orders`라는 이름으로, 일별 총 주문 금액을 `total_amount`라는 이름으로 포함해서 조회하세요.
        ...
        ```
*/
SELECT o.date, COUNT(DISTINCT o.id) AS total_orders, SUM(od.quantity * p.price) AS total_amount
FROM orders AS o
JOIN order_details AS od ON o.id = od.order_id
JOIN pizzas AS p ON p.id = od.pizza_id
WHERE o.date >= '2025-03-01' AND o.date <= '2025-03-31'
GROUP BY o.date
ORDER BY o.date ASC;
...
/*
    3. `order`의 `id`가 78에 해당하는 주문 내역들을 조회합니다. 주문 내역에서 각각 주문한 피자의 이름을 `pizza_name`, 피자의 크기를 `pizza_size`, 피자 가격을 `pizza_price`, 수량을 `quantity`, 각 주문 내역의 총 금액을 `total_amount` 라는 이름으로 조회해 주세요.
        ...
        ```
*/
SELECT pt.name AS pizza_name, p.size AS pizza_size, p.price AS pizza_price, od.quantity, od.quantity * p.price AS total_amount
FROM orders AS o
JOIN order_details AS od ON od.order_id = o.id
JOIN pizzas AS p ON p.id = od.pizza_id
JOIN pizza_types AS pt ON pt.id = p.type_id
WHERE o.id = 78;
...
/*    
    4. `order_details`와 `pizzas` 테이블을 JOIN해서 피자 크기별(S, M, L) 총 수익을 계산하고, 크기별 수익을 출력하세요.
        ...
        ```
*/
SELECT p.size, SUM(od.quantity * p.price) AS size_total_amount
FROM order_details AS od
JOIN pizzas AS p ON p.id = od.pizza_id
GROUP BY p.size
ORDER BY size_total_amount DESC;
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
SELECT pt.name, SUM(od.quantity * p.price) AS total_amount
FROM order_details AS od
JOIN pizzas AS p ON p.id = od.pizza_id 
JOIN pizza_types AS pt ON pt.id = p.type_id
GROUP BY pt.name
