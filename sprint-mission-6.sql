...
SELECT * FROM orders
WHERE date = '2025-03-12' AND time BETWEEN '00:00:00' AND '11:59:59';
...
SELECT pizza_id, COUNT(order_id) AS total_orders FROM order_details GROUP BY pizza_id;
...
SELECT od.pizza_id, SUM(od.quantity * p.price) AS total_revenue FROM order_details AS od JOIN pizzas AS p ON od.pizza_id = p.id GROUP BY pizza_id ORDER BY total_revenue DESC;
-- 7. 날짜별로 피자 주문 건수(`order_count`)와 총 주문 수량(`total_quantity`)을 구하세요.
SELECT o.date, COUNT(DISTINCT o.id) AS order_count, SUM(od.quantity) AS total_quantity FROM orders AS o JOIN order_details AS od ON o.id = od.order_id GROUP BY o.date;
...
/*    
    4. `order_details`와 `pizzas` 테이블을 JOIN해서 피자 크기별(S, M, L) 총 수익을 계산하고, 크기별 수익을 출력하세요.
        ...
        ```
*/
SELECT p.size AS pizza_size, SUM(od.quantity * p.price) AS total_amount
FROM pizzas AS p
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
SELECT pt.name AS pizza_name, SUM(od.quantity * p.price) AS total_amount
