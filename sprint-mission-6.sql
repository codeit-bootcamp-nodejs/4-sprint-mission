...
-- 7. `orders` 테이블에서 커서 페이지네이션된 목록을 조회합니다. 페이지 크기가 10이고 최신순일때, `id` 값을 기준으로 커서를 사용합시다. 
...
SELECT  quantity * (SELECT price FROM pizzas ) AS total_revenue
FROM order_details
GROUP BY pizza_id
