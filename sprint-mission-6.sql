...
AND time BETWEEN '00:00:00' AND '11:59:59';
...
SELECT
  o.date, COUNT(DISTINCT o.id) AS order_count,
...
FROM pizzas AS p
INNER JOIN order_details AS od ON p.id = od.pizza_id
GROUP BY p.id
ORDER BY total_quantity DESC
LIMIT 10;
/*
...
GROUP BY o.date
ORDER BY o.date ASC, total_orders DESC, total_amount DESC;
...
/*    
    4. `order_details`와 `pizzas` 테이블을 JOIN해서 피자 크기별(S, M, L) 총 수익을 계산하고, 크기별 수익을 출력하세요.
        ...
        ```
*/
SELECT p.size AS pizza_size, sum(od.quantity * p.price) AS total_revenue
FROM order_details AS od
INNER JOIN pizzas AS p ON od.pizza_id = p.id
GROUP BY pizza_size
ORDER BY total_revenue DESC;
