...
### User 스키마
- `User` 스키마를 작성해 주세요.
- 필수 필드: `id`, `email`, `nickname`, `image`, `password`, `createdAt`, `updatedAt`
...
- 상품 또는 게시글을 조회할 때, 유저가 '좋아요'를 누른 항목인지 확인할 수 있도록 `isLiked`와 같은 불린형 필드를 리스폰스 객체에 포함시켜 리스폰스해 주세요.
- 유저가 '좋아요'를 표시한 상품의 목록을 조회하는 기능을 구현합니다.

## 참고 자료
좋아요 구현이 막막하다면 다음 문서를 참고해 보세요:
- [Prisma의 Relations](https://www.prisma.io/docs/concepts/components/prisma-schema/relations)
- [Prisma의 include 옵션](https://www.prisma.io/docs/concepts/components/prisma-client/field-selection#include-relations-and-select-fields)

## 기술 스택
- Node.js (ES Module 환경)
- Prisma
- JWT (JSON Web Token)
- 비밀번호 해싱 라이브러리 (예: bcrypt)