...
describe("Product Comment API 통합 테스트 (JWT 기반)", () => {
  const testUser = {
  ...
  describe("POST /products/:productId/comments", () => {
    ...
    it("존재하지 않는 상품에 댓글 등록 시 404 Not Found 반환", async () => {
      const nonExistentProductId = 9999;
      await request(app)
        .post(`/products/${nonExistentProductId}/comments`)
        .set("Cookie", cookie)
        .send({ content: "댓글 내용" })
        .expect(404);
    });
  ...
  describe("GET /products/:productId/comments", () => {
    ...
    it("limit=1 페이지네이션 테스트 (최신 댓글 1개만 조회)", async () => {
      const res = await request(app)
        .get(`/products/${productId}/comments?limit=1`)
        .expect(200);
    ...
    it("선택한 범위에 댓글이 없을 경우 404 Not Found 반환", async () => {
      ...
      await request(app)
        .get(`/products/${productId}/comments?cursor=${tooSmallCursor}&limit=1`)
        .expect(404);
    });
  });

  describe("PATCH /products/:productId/comments/:commentId", () => {
    ...
    it("존재하지 않는 댓글 수정 시 404 Not Found 반환", async () => {
      ...
      await request(app)
        .patch(`/products/${productId}/comments/${nonExistentCommentId}`)
        .set("Cookie", cookie)
  ...
  describe("DELETE /products/:productId/comments/:commentId", () => {
    ...
    it("다른 유저의 댓글 삭제 시 403 Forbidden 반환", async () => {
      ...
      await request(app)
        .delete(`/products/${productId}/comments/${commentId2}`)
        .set("Cookie", user2Cookie)
        .expect(403);
    });
    ...
    it("존재하지 않는 댓글 삭제 시 404 Not Found 반환", async () => {
      const nonExistentCommentId = 9999;
      await request(app)
        .delete(`/products/${productId}/comments/${nonExistentCommentId}`)
        .set("Cookie", cookie)
        .expect(404);
    });
