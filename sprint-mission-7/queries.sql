...
*/
UPDATE "User"
SET nickname = 'test', updatedAt = NOW()
WHERE id = 1;
...
SELECT p.*
FROM "Product" AS p
INNER JOIN "ProductLike" AS pl ON p.id = pl.productId
WHERE pl.userId = 1
...
/*
  ...
  - 현재 로그인한 유저 id가 1이라고 가정
*/
SELECT COUNT(*) AS liked_product_total
FROM "Product" AS p
INNER JOIN "ProductLike" AS pl ON p.id = pl.productId
WHERE pl.userId = 1;
...
/*
  ...
  - 현재 로그인한 유저 id가 1이라고 가정
*/
INSERT INTO "Product" (name, description, price, tags, userId)
VALUES
('test', '이 상품은 테스트용으로 생성되었습니다.', 15000, ARRAY['test','new'], 1);
...
FROM "Product" AS p
LEFT JOIN "ProductLike" AS pl ON p.id = pl.productId
...
/*
  ...
  - 1번 유저가 2번 상품에 댓글 작성
*/
INSERT INTO "ProductComment" (content, productId, userId)
VALUES ('상품이 좋아요', 2, 1);
