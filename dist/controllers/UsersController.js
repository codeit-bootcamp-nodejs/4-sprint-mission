    ...
    constructor(userService) {
        ...
        this.changePassword = async (req, res, next) => {
            try {
                ...
                const isPasswordMatched = await bcrypt_1.default.compare(currentPassword, existingUser.password);
        ...
        this.refreshToken = async (req, res, next) => {
            try {
                ...
                const decodedToken = jsonwebtoken_1.default.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET_KEY);
                const userId = decodedToken.userId;
                const user = await this.userService.getUserById(userId);
                if (!user) {
                    return res.status(401).json({ message: '사용자를 찾을 수 없습니다.' });
                }
                const isRefreshTokenMatched = await bcrypt_1.default.compare(refreshToken, user.refreshToken);
                if (!isRefreshTokenMatched) {
                    return res.status(401).json({ message: 'Refresh Token이 유효하지 않습니다.' });
                }
                const newAccessToken = jsonwebtoken_1.default.sign({ userId: user.id }, process.env.JWT_SECRET_KEY, {
                    expiresIn: '12h',
                });
                return res.status(200).json({
